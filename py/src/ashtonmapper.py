...
from tablut.ashton import AshtonBoard
...
class AshtonMapper:
    ...
    def map_to_protocol_action(self, state: game.State, action: game.Action) -> Action:
        return Action(Action.map_to_coord(*action.from_),
                      Action.map_to_coord(*action.to),
    ...
    def map_to_game_state(self, state: State) -> game.State:
        ...
        return tablut.TablutState(AshtonBoard(board), turn)

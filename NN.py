# -*- coding: utf-8 -*-
"""
Created on Sun Nov 14 17:00:07 2021
...
for i in range(len(states)):
    for j in range(len(states[i])):
        states[i][j]=ch(states[i][j])

for i in range(len(states)):
    states[i]=np.array(states[i])
    states[i]=states[i].reshape(81,1)
...
#rendo w una lista di vettori del tipo (1,0,0)
winners=[np.array([[int(w[i]=='WW')],[int(w[i]=='D')],[int(w[i]=='BW')]]) for i in range(len(w))]
del w

#espando i vincitori su gli stati
game_id=list(df['game_id'])
wwinners=[]
for i in range(len(game_id)):
    wwinners.append(winners[game_id[i]-1])

data=list(zip(states,wwinners))
...
DEFAULT_LAYER_SIZES = [9*9, 16, 16, 3]
sizes=DEFAULT_LAYER_SIZES
...
for i in range(len(test)):
    nres=nn.feedforward(test[i][0])
    ntrue=test[i][1]
    c.append(np.sum((nres-ntrue)**2))
